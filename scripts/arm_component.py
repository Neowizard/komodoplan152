...
default_values = {
    ...
    "block_height": 4,
    "arm_length": 50,
    "right_finger_released_angle": 1,
    "left_finger_released_angle": -1,
    "init_left_finger_angle": -0.15,
    "init_right_finger_angle": 0.15,
    "finger_level_offset": 0.1
}
...
class KomodoPicknPlaceComp:
    ...
    def handle_pick_up(self, pick_up_msg):
        ...
        if (query_result is None):
            ...
            self.position_arm(block_pos.tablePos, block_pos.level)
    ...
    def handle_put_down(self, put_down_msg):
        ...
        if (query_result is None):
            ...
            self.position_arm(on_block_pos.tablePos, on_block_pos.level + 1)
    ...
    def position_arm(self, table_pos, height_level):
        ...
        base_rot_publisher = rospy.Publisher("/komodo_1/base_controller/command", Float64, queue_size=10)
        ...
        if (wrist_angle > default_values["wrist_max_angle"]):
            wrist_angle -= math.pi
        wrist_rot_publisher = rospy.Publisher("/komodo_1/wrist_controller/command", Float64, queue_size=10)
        ...
        shoulder_rot_publisher = rospy.Publisher("/komodo_1/shoulder_controller/command", Float64, queue_size=10)
        shoulder_angle_msg = Float64()
        shoulder_angle_msg.data = shoulder_angle
        shoulder_rot_publisher.publish(shoulder_angle_msg)

        # Position elbow 2
        elbow_angle = default_values["elbow_min_angle"] + shoulder_angle
        elbow_rot_publisher = rospy.Publisher("/komodo_1/elbow2_controller/command", Float64, queue_size=10)
    ...
    def get_base_angle(self, table_pos):
        ...
        base_angle = min_rotation_angle + (max_rotation_angle - min_rotation_angle) * \
                                     ((table_pos - 1) / (self.table_positions_count - 1))
